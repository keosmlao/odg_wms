...
/**
 * ຮັບຄືນ — the SOURCE warehouse takes back goods it issued into 9903 that the
 * destination never received (e.g. wrong/over-issue). Moves 9903 → source.
 *
 * GET                → transfers with goods still in 9903 issued FROM a source the user owns
 * GET ?doc=<124>     → that transfer's lines (in-transit) + serials in 9903
 * POST {doc, location_to, lines:[{item_code, qty, serials}]} → return to source
 */
const REQ_FLAG = 124;
...
export async function GET(request: Request) {
  ...
  // History: completed returns (DP docs out of 9903 back into a source the user owns).
  if (url.searchParams.get("history")) {
    ...
    const hist = await query<{ doc_no: string; doc_date: string | null; doc_time: string | null; doc_ref: string | null; wh_code: string | null; wh_name: string | null; items: number; qty: string }>(
      `SELECT t.doc_no, to_char(t.doc_date,'YYYY-MM-DD') AS doc_date, t.doc_time, t.doc_ref, t.wh_code, w.name_1 AS wh_name,
       ...
       JOIN public.ic_trans r ON r.doc_no = t.doc_ref AND r.trans_flag = ${REQ_FLAG} AND r.wh_from = t.wh_code
       JOIN LATERAL (
  ...
  if (doc) {
    ...
    const lines = await query<{ item_code: string; item_name: string | null; unit_code: string | null; in_transit: string; returned: string; serialized: boolean }>(
      `SELECT t.item_code,
              ...
              COALESCE(SUM(t.qty) FILTER (WHERE t.calc_flag = 1 AND t.wh_code = $3), 0)::numeric::text AS returned,
      ...
      [doc, IN_TRANSIT_WH, head[0]?.wh_from ?? ""],
    );
    ...
    const dest = head[0]?.wh_from ?? ""; // return lands at source
    ...
    const sameLocs = items.length > 0 ? await query<{ location: string; item_code: string; qty: string }>(
      `SELECT NULLIF(TRIM(shelf_code1), '') AS location, item_code, SUM(qty * calc_flag)::numeric::text AS qty
       FROM public.odg_wms_trans_detail
       WHERE wh_code = $1 AND item_code = ANY($2) AND NULLIF(TRIM(shelf_code1), '') IS NOT NULL AND (status = 0 OR status IS NULL)
...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(body.lines) || body.lines.length === 0) return NextResponse.json({ error: "ບໍ່ມີລາຍການໃຫ້ຮັບຄືນ" }, { status: 400 });
  ...
  if (lines.length === 0) return NextResponse.json({ error: "ບໍ່ມີລາຍການໃຫ້ຮັບຄືນ" }, { status: 400 });

  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const head = await client.query<{ wh_from: string | null }>(
      `SELECT wh_from FROM public.ic_trans WHERE doc_no = $1 AND trans_flag = ${REQ_FLAG} LIMIT 1`,
      [doc],
    );
    const whFrom = head.rows[0]?.wh_from ?? "";
    if (!whFrom) { await client.query("ROLLBACK"); return NextResponse.json({ error: "ບໍ່ພົບໃບຂໍໂອນ" }, { status: 404 }); }
    const accessible = accessibleWarehouses(session);
    ...
    const notes = Array.isArray(body.notes)
      ...
      : [];
    // Return lands the goods back in the SOURCE warehouse.
    const res = await moveFromTransit(client, { refDoc: doc, whTo: whFrom, locTo, user: session.employee_code ?? null, stage: "return", lines, notes });
