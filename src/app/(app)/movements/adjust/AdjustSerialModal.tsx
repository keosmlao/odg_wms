      ...
      <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-zinc-200 dark:bg-zinc-900 dark:ring-zinc-800" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-3 border-b border-zinc-100 px-5 py-3 dark:border-zinc-800">
          <div className="min-w-0">
            <div className="font-mono text-xs font-bold text-indigo-600 dark:text-indigo-400">{item.item_code}</div>
        ...
        <div className="border-b border-zinc-100 px-5 py-1.5 dark:border-zinc-800">
          <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="ຄົ້ນຫາ ISN / location..." className="w-full rounded-lg bg-white px-2.5 py-1 text-xs ring-1 ring-zinc-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-zinc-950 dark:ring-zinc-800" />
        </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-zinc-100 px-5 py-3 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => onDone({ serialsRemove: [...removeSet], serialsAdd: addList.map((a) => a.code), serialsGenerate: generate })} className="rounded-lg bg-gradient-to-r from-indigo-500 to-violet-600 px-5 py-2 text-xs font-semibold text-white shadow-sm">ນຳໃຊ້ (Δ {delta > 0 ? "+" : ""}{delta})</button>
          </div>
        </div>
      </div>
    </div>
