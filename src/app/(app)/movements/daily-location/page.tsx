...
import LocationDailyClient, { type WarehouseOption } from "./LocationDailyClient";

export default async function DailyLocationPage() {
  ...
  return (
    <div className="w-full space-y-5">
      <Hero
        title="ເຄື່ອນໄຫວປະຈຳວັນ ຕາມບ່ອນເກັບ"
        description="ຍອດຍົກມາ · ຮັບເຂົ້າ · ຈ່າຍອອກ · ຄົງເຫຼືອ — ແຍກເປັນຊັ້ນວາງ ແລະ ບ່ອນເກັບ, ກົດແຖວເພື່ອເບິ່ງຕາມມື້ ແລະ ຕາມສິນຄ້າ"
        icon={<MapPinIcon className="h-6 w-6" />}
        tone="aqua"
        chips={<Chip tone="primary">{ROLE_LABEL_LO[session.role]}</Chip>}
      />
      <LocationDailyClient warehouses={warehouses} />
    </div>
  );
}
