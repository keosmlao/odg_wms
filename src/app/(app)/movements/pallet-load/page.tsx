...
import PalletLoadClient, { type WarehouseOption } from "./PalletLoadClient";

export default async function PalletLoadPage() {
  ...
  return (
    <div className="w-full space-y-5">
      <Hero
        ...
        description="ເລືອກ pallet ແລ້ວ ສະແກນ/ເພີ່ມ ສິນຄ້າ ແລະ serial ເຂົ້າ pallet — stock ເພີ່ມຢູ່ບ່ອນຂອງ pallet"
        icon={<PackageIcon className="h-6 w-6" />}
        tone="emerald"
        chips={<Chip tone="primary">{ROLE_LABEL_LO[session.role]}</Chip>}
      />
      <PalletLoadClient warehouses={warehouses} />
    </div>
  );
}
